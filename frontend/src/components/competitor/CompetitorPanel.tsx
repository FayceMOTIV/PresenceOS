  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        ...
        <div className="bg-gray-100/60 rounded-lg p-2">
          <div className="text-sm font-bold text-gray-900">{competitor.engagement_rate}%</div>
          <div className="text-[10px] text-gray-500">Engagement</div>
        </div>
        <div className="bg-gray-100/60 rounded-lg p-2">
          <div className="text-sm font-bold text-gray-900">{competitor.post_frequency}/sem</div>
          <div className="text-[10px] text-gray-500">Frequence</div>
        </div>
      </div>
...
export function CompetitorPanel({ brandId }: CompetitorPanelProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      {/* Benchmark summary */}
      {benchmark && (
        <div className="bg-white border border-gray-200/60 rounded-xl p-5 space-y-4 shadow-sm">
          <div className="flex items-center gap-2">
            <Target className="w-5 h-5 text-amber-400" />
            <h3 className="text-sm font-semibold text-gray-900">Benchmark concurrentiel</h3>
          </div>
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              return (
                <div key={m.label} className="bg-gray-100/60 rounded-lg p-3 text-center">
          ...
          {benchmark.gaps.length > 0 && (
            <div className="space-y-1">
              <p className="text-[10px] text-gray-400 uppercase tracking-wider">Recommandations</p>
              {benchmark.gaps.map((gap, i) => (
                <p key={i} className="text-xs text-gray-600">• {gap}</p>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-black text-xs font-medium transition-colors"
        >
      ...
      {showAdd && (
        ...
        >
          <input
            ...
            placeholder="Nom"
            className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:border-amber-500/50 focus:outline-none"
          />
          <input
            value={newHandle}
          ...
          <button
            onClick={handleAdd}
            className="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-black text-sm font-medium transition-colors"
          >
            OK
          </button>
        </motion.div>
