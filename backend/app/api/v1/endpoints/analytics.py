...
async def get_overview(
    ...
    return service.get_overview(str(brand.id), days)
...
async def get_kpis(
    ...
    service = _get_service()
    return service.get_kpis(str(brand.id), days)
...
async def get_timeline(
    ...
    service = _get_service()
    return service.get_timeline(str(brand.id), days)


@router.get("/insights/{brand_id}")
async def get_insights(brand: CurrentBrand):
    """Get weekly AI-generated insights and recommendations."""
    service = _get_service()
    return service.get_insights(str(brand.id))
